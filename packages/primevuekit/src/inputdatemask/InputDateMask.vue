<template>
    <div class="flex gap-2" :class="props.class">
        <InputMask :mask="maskValue" v-model="inputMaskRef" class="w-full" :id="props.inputId" :placeholder="actualPlaceholder" @blur="handleInputBlur" @input="handleInputChange" />
...
const convertDateFormat = (format: string) => {
    ...
    return format.replace(/yyyy/g, 'yyyy').replace(/yy/g, 'yy').replace(/mm/g, 'MM').replace(/dd/g, 'dd');
};
...
watch(
    ...
    (newValue) => {
        ...
        inputMaskRef.value = formatDate(dateValue);
        emitValueChange(dateValue);
        nextTick(() => {
            // 确保输入框更新后，Popover 可以正确显示
            if (op.value) {
                op.value.hide();
            }
        });
